...
c
     ...
     & Inline_Ix_Emissions,Root_Output_Name,Average_Output_3D,
...
c
     ...
     & Partial_Source_Area_Map,SA_PT_Override
c
...
c      
      ...
      logical   lexist, lacross, lmech2_ok, lmech3_ok, lmech4_ok
      logical   lmech5_ok, lmech6_ok, lmech10_ok
      logical   lixemspc
c
      data inp /3/
      data ctlfil /'CAMx.in'/
...
c
      ...
      if( idmech.EQ.2 .AND. .NOT. lmech2_ok ) then
         write(iout,'(//,a)') 'ERROR in READNML:'
         write(iout,'(/,2A)') ' You have selected a probing tool that ',
     &               'is currently not supported with Mechanism 2.'
...
c
     ...
     &                'choose the CF Aerosol Scheme with SOA chemistry.'
          call camxerr()
      endif
c
...
c
     ...
     &                     ' with the PiG model'
            ...
            if (lparttn .AND. .not.lsrfmodrt) then
              write(iout,*)'The RTRAC Surface Model must be used',
...
c
 7102 continue
     ...
     &                ' namelist variables have changed.'
      write(*,'(/A)') '   The following variables:'
      write(*,'(A)')  '       POLAR_Longitude_Pole'
      write(*,'(A)')  '       POLAR_Latitude_Pole'
      write(*,'(A)')  '       LAMBERT_Center_Longitude'
      write(*,'(2A)') '       LAMBERT_Center_Latitude'
      write(*,'(2A)') '       LAMBERT_True_Latitude1'
      write(*,'(2A)') '       LAMBERT_True_Latitude2'
      write(*,'(A)')  '   Have been replaced with:'
      write(*,'(2A)') '       Longitude_Pole'
      write(*,'(2A)') '       Latitude_Pole'
      write(*,'(2A)') '       True_Latitude1'
      write(*,'(2A)') '       True_Latitude2'
      write(*,'(/,A)')'   The following variables:'
      write(*,'(2A)') '       Master_Origin_XCoord'
      write(*,'(2A)') '       Master_Origin_YCoord'
      write(*,'(A)')  '   Have been replaced with:'
      write(*,'(A)')  '        Master_SW_XCoord'
      write(*,'(A)')  '        Master_SW_YCoord'
      write(*,'(2A,/,A)') ' Also, this version no longer supports HDF format.',
     &                ' Please remove the',' HDF_Format_Output flag.'
      write(*,'(/,A,/,A,//)') 
...
c
 7103 continue
     ...
     &                   'variables have changed'
      write(*,'(1X,2A)') 'significantly.'
      write(*,'(1X,2A)') 'In particular, the OSAT/APCA options and ',
     &           'PSAT options have been consolidated.'
      write(*,'(1X,2A)') 'Include the following for all ',
     &           'source apportionment types: OSAT/APCA/PSAT'
      write(*,'(1X,A)') '    SA_Treat_SULFATE_Class'
      write(*,'(1X,A)') '    SA_Treat_NITRATE_Class'
      write(*,'(1X,A)') '    SA_Treat_SOA_Class'
      write(*,'(1X,A)') '    SA_Treat_PRIMARY_Class'
      write(*,'(1X,A)') '    SA_Treat_MERCURY_Class'
      write(*,'(1X,A)') '    SA_Treat_OZONE_Class'
      write(*,'(1X,A)') '    SA_Use_APCA'
