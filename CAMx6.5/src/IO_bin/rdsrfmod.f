...
c
      ...
      if (igrd.gt.1) then
        ...
        orgx = orgx + dx*(inst1(igrd)-1) - dxf
        orgy = orgy + dy*(jnst1(igrd)-1) - dyf
        dx = dxf
        dy = dyf
      endif
      ...
      do isp = 1,nsurf
         mspec(1,isp) = 'S'
         read(smspc(isp)(1:9),'(9A1)') (mspec(i,isp),i=2,10)
         srfmodname(isp) = 'S'//smspc(isp)(1:9)
      enddo
      do isp = 1,nsurf
         mspec(1,isp+nsurf) = 'V'
         read(smspc(isp)(1:9),'(9A1)') (mspec(i,isp+nsurf),i=2,10)
         srfmodname(isp+nsurf) = 'V'//smspc(isp)(1:9)
      enddo
c
      if( lcdfout) then
     ...
     &                                     spec_coords,2*nsurf,srfmodname)
      ...
      if( .NOT. lcdfout ) then
         ...
         write(iunit) plon,plat,iutm,orgx,orgy,dx,dy,nox,noy,
