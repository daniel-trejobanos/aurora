...
c-----------------------------------------------------------------------
c
      integer i, idxcel, idx, jdx, kdx, ldx, idx0, idx2
...
c
      if( lsoa ) then
         delARO = delcon(1,ktol) + delcon(1,kxyl) + delcon(1,kbenz) ! [umol/m3] for gases
     &                                            + delcon(1,kivoa)
...
c
         ...
         do i=iptcls(idxipt(ITRCG3)),nptcls(idxipt(ITRCG3))
            ...
            itrp = itrp + 1
            kdx = idxcel + numcol * numrow * numlay * (itrp-1)
            isqt = isqt + 1
            ldx = idxcel + numcol * numrow * numlay * (isqt-1)
