...
c
      ...
      write(11,900) fname,note,nseg,nspec,idate,begtim,jdate,endtim
...
c---  region definition ---
c
      read (10) orgx,orgy,iutm,utmx,utmy,dx,dy,nx,ny,
     &          nz,nzlo,nzup,hts,htl,htu
      write(11,901) orgx,orgy,iutm,utmx,utmy,dx,dy,nx,ny,
     &               nz,nzlo,nzup,hts,htl,htu
      write(*,901) orgx,orgy,iutm,utmx,utmy,dx,dy,nx,ny,
     &               nz,nzlo,nzup,hts,htl,htu
      read (10) i1,j1,nx1,ny1
      write(11,902) i1,j1,nx1,ny1
c
c---  species records ---
c
      if (lhdrspec) then
        read (10) ((spname(m,l),m=1,10),l=1,nspec)
        write(11,903) ((spname(m,l),m=1,10),l=1,nspec)
      endif
c
...
c
      if (lbndry) then
        do i=1,4
          read (10) iseg, iedge, ncells,
     &            ((iloc(ii,j),ii=1,4),j=1,ncells)
          write(11,908) iseg, iedge, ncells,
     &            ((iloc(ii,j),ii=1,4),j=1,ncells)
        enddo
      endif
c
...
c
      if (lptsrc) then
        read (10) iseg,npmax
        write(11,910) iseg,npmax
        if (npmax.gt.MXPT) then
          write(*,*) 'Increase MXPT to ',npmax,' and recompile'
          stop
        endif
        if (npmax.gt.0) then
          read (10) (xc(ip),yc(ip),sh(ip),sd(ip),st(ip),sv(ip),
     &               ip=1,npmax)
          do ip=1,npmax
            write(11,911) xc(ip),yc(ip),sh(ip),sd(ip),st(ip),sv(ip)
          enddo
        endif
      endif
c
...
c
      do 100 ihr = 1,999
        ...
        write(11,904) ibgdat,begtim,iendat,endtim
c
c---  boundary file ---
c
        if (lbndry) then
          do 130 l=1,nspec
            ...
            do 140 i=1,4
              if (i.ge.3) ncel= nx
              read (10,err=800) iseg,(spname(m,l),m=1,10),nedgno,
     &            ((bconc(k,ij),k=1,nz),ij=1,ncel)
              write(11,909) iseg,(spname(m,l),m=1,10),nedgno,
     &            ((bconc(k,ij),k=1,nz),ij=1,ncel)
  140       continue
  130    continue
c
...
c
        elseif (lptsrc) then
          read (10) iseg,numpts
          write(11,910) iseg,numpts
          if (numpts.gt.0) then
            read (10)     (icell(ip),jcell(ip),kcell(ip),flow(ip),
     &                     plumht(ip),ip=1,numpts)
            write(11,912) (icell(ip),jcell(ip),kcell(ip),flow(ip),
     &                     plumht(ip),ip=1,numpts)
            do l=1,nspec
              read (10) iseg, (spname(m,l),m=1,10), (emiss(ip),
     &                          ip=1,numpts)
              write(11,905) iseg, (spname(m,l),m=1,10)
              write(11,913) (emiss(ip),ip=1,numpts)
            enddo
          endif
c
...
c
          ...
          do 110 l = 1,nspec
            do 120 k = 1,nlay
              read (10,err=800) idum, (spname(m,l),m = 1,10), 
     &                        ((temp(i,j),i=1,nx),j=1,ny)
              write(11,905) idum, (spname(m,l),m=1,10)
              write(11,906) ((temp(i,j),i=1,nx),j=1,ny)
 120        continue    
 110      continue    
        endif
c
