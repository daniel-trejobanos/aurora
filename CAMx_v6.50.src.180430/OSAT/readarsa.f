...
c
      allocate( lefcls(MXTRCLS,MXCELLS,MXCELLS) )
      allocate( emscls(MXTRCLS,MXCELLS,MXCELLS) )
c
...
c
     ...
     &                                  igroup,' = ', emscls(icls,i,j)
                   ...
                   emisar(i,j,ipos) = emisar(i,j,ipos ) + lefcls(icls,i,j) * frac
                enddo
   60        continue
   50     continue
       endif
