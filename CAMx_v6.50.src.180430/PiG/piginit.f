      subroutine piginit(numprocs,dt,npts,nrads,nspc,pttrace,height,windu,
     &                                         windv,tempk,press)
      use filunit
      use grid
      use chmstry
...
c
      ...
      do 50 lsrc = 1,nosrc(1)
        ...
        iempty = 0
        do l = 1,nspc
          if( lptmap(l) .GT. 0 .AND. pttrace(n,l) .GT. 0.) iempty = 1
        enddo
c
