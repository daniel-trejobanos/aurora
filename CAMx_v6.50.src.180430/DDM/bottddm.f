      subroutine bottddm(nn,dt,dx,nspcs,sen,nsen,con0,con,
     &                                          fpc,fmc,vel,area,av1d)
      use tracer
c  
c----CAMx v6.50 180430
c  
c     BOTTDDM performs horizontal advection of the sensitivity coefficients.
...
c        nspcs               number of tracer species  
c        sen                 Sensitivity coefficient vector 
c                              (umol/m3/parameter unit) 
c        nsen                Number of sensitivity coefficients
c        con0                Concentration vector (umol/m3) at start of
c                            timestep
c        con                 Concentration vector (umol/m3) at end of
c                            timestep
c        fpc, fmc            Positive and negative concentration fluxes before
c                            the flux limiting renormalization step (umol/m)
c        vel                 Area-weighted wind speed vector (m/s)  
c        area                Area adjustment vector
c        av1d                Interfacial area adjustment vector
c  
c     Output arguments:  
c        sen                 Sensitivity coefficient vector 
c                              (umol/m3/parameter unit)  
...
c
      real sen(MXCELLS,*)
      real con0(nn),con(nn),fpc(nn),fmc(nn),vel(nn),area(nn),av1d(nn)
c
      real a0(MXTRSP)
      real a1(MXTRSP)
      real a2(MXTRSP)
      ...
      real a4(MXTRSP)
      real cn(MXCELLS)
      real fm(MXCELLS,MXTRSP)
      real fp(MXCELLS,MXTRSP)
c
      data small / 1.e-20/
c             
c-----Entry point 
...
c-----Test if negative or small concentration was reset in hadvbot to 1.0e-20.
        if (con(i).gt.1.0e-20) then
          do j=1,nsen
            sen(i,j) = sen(i,j) - area(i)*
     &                            (av1d(i)*(fp(i,j) - fm(i+1,j)) - 
     &                             av1d(i-1)*(fp(i-1,j) - fm(i,j)))
          enddo
        else
          do j=1,nsen
            sen(i,j) = 0.0
