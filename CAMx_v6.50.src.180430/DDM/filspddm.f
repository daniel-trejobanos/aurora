...
c
         if( lcdfout ) then
            ddmdesc(iptfam) = 'Second order sesitivity to '//
     &               caffec(:istrln(caffec))//' from '//
     &                  hddmsp(1,ngrps)(:istrln(hddmsp(1,ngrps)))//'&'//
     &                        hddmsp(2,ngrps)(:istrln(hddmsp(2,ngrps)))
         endif
c
         goto 9999
      endif
...
c
     ...
     &                                  ptlong(iptfam), ptname(iptfam)
         if( lcdfout ) then
            ddmdesc(iptfam) = 'Sesitivity to '//caffec(:istrln(caffec))//
     &                            ' from '//cinflu(:istrln(cinflu))//' rate'
         endif
...
c
     ...
     &                                  ptlong(iptfam), ptname(iptfam)
                if( lcdfout ) then
                   write(ddmdesc(iptfam),'(6A,I2.2,A,I2.2)') caffec(:istrln(caffec))
     &                ,' from ',srcnam(:istrln(srcnam)),' ',cinflu(:istrln(cinflu)),
     &                                                        ' G=',igrp,'R=',ireg
                endif
            enddo
         enddo
...
c
     ...
     &                                  ptlong(iptfam), ptname(iptfam)
         if( lcdfout ) then
              write(ddmdesc(iptfam),'(6A,I2.2,A,I2.2)') caffec(:istrln(caffec))
     &             ,' from ',cinflu(:istrln(cinflu)),' ',srcnam(:istrln(srcnam))
         endif
