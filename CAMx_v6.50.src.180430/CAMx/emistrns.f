...
c
      if (ipigflg .NE. 0 .AND. ipigint .EQ. igrd) then
        if( iproc_id .eq. 0 ) then
           ...
           call piginit(numprocs,deltat(igrd),nptsrc,nrad,nspec,ptemis,
     &                  height,windu,windv,tempk,press)
           write(*,'(a)') '   Done'
           call flush(6)
           write(iout,'(a)') '   Done'
           call flush(iout)
...
c
      ...
      if( .NOT. lmpi .OR. (iproc_id .GT. 0) ) then
        ...
        call emiss(mxp,myp,mzp,i0,j0,ia,iz,ja,jz,
     &           nspec,nlu,3*ndepspc+2,densfac,
     &           larmap(1,igrd),lptmap,.FALSE.,
...
c
        if( ltrace .OR. ((lddm .OR. lhddm) .AND. lddmcalc(igrd)) ) then
           call emiss(mxp,myp,mzp,i0,j0,ia,iz,ja,jz,
     &              ntotsp,nlu,1,densfac,lsamap,lsamap,.TRUE.,
...
c
     ...
     &             ctop(iptrem(igrd)),pttop,fluxes(1,igrd),ipsa3d(igrd),
...
c
     ...
     &              tectyp .NE. RTRAC .AND. tectyp .NE. RTCMC ) then
              call filptopsa(igrd,mxp,myp,nspec,ntotsp,
     &                       ctop(iptrem(igrd)),pttop)
          endif
c
c======================== Source Apportion End =======================
c
...
c
     ...
     &             tempk(iptr3d(igrd)),press(iptr3d(igrd)),
     &             spname,conc(iptr4d(igrd)),rhoscl,ctop(iptrem(igrd)),
