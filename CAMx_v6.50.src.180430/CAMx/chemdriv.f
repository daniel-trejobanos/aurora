...
c
      ...
      external hr_xo3, hr_xo4, hr_xo_dum
      external lsrate2
      external lsrate3
      external lsrate4
      external lsrate5
...
c
            if( aero_flag .GT. 0 ) then
              if ( lvbs ) then
                if (idmech.eq.2 .or. idmech.eq.3
     &                          .or. idmech.eq.4) then ! cb6
...
c
            if ( lhddm .AND. lddmcalc(igrd) ) then
              if (idmech.eq.2) then
                call hddmchem(hddmjac2,ebirxn2,ebirate2,ddmrate2,
...
c
     ...
     &                       con_CG(:NVBS*NSET),con_NV(:NSET))
                ...
                if (idmech.ne.5 .and. kSQT.lt.nspec+1)
...
c
                if ( lddm ) then
                  if (idmech.ne.5 .and. kSQT.lt.nspec+1) then
                    do is = 1, nddmsp
...
c
            if( lirr .AND. (l3dflag .OR. k .EQ. 1) ) then
               ...
               dtfact = dt/dtout/60.
               if( idmech.EQ.2 ) then
                 call cpamech2(rrxn_irr,rk,dtfact,MXREACT,patmp,ntotsp,
     &                                                    nn,ldark(i,j))
               elseif( idmech.EQ.3 ) then
