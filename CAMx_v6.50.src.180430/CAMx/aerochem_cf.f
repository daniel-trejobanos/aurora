...
c
          if (lddm) then
            call DDMSOAP (dtaer,convfac,sddm,nfam,nsen,ierr,iout)
            if (ierr.lt.0) then
...
c
        ...
        if (kPCA.lt.nspec+1) then
          r_aer(4) = (con(kPCA)/wtmol(8)/convfac)*1.e-6
        elseif (f_CA_PRM.ge.0. .and. f_CA_CRS.ge.0.) then
          r_aer(4) = ((f_CA_PRM*con(kFPRM)+
     &                 f_CA_CRS*con(kFCRS))/wtmol(8)/convfac)*1.e-6
        else
          r_aer(4) = (caco3/(wtmol(8)+wtmol(7))/convfac)*1.e-6
        endif

        if (kPMG.lt.nspec+1) then
          r_aer(5) = (con(kPMG)/wtmol(9)/convfac)*1.e-6
        elseif (f_MG_PRM.ge.0. .and. f_MG_CRS.ge.0.) then
          r_aer(5) = ((f_MG_PRM*con(kFPRM)+
     &                 f_MG_CRS*con(kFCRS))/wtmol(9)/convfac)*1.e-6
        else
        ...
        if (kPFE.lt.nspec+1 .or. (f_FE_PRM.ge.0. .and. f_FE_CRS.ge.0.)) then
          if ( ldark ) then
          ...
          if (kPFE.lt.nspec+1) then
            fe_mmr = (con(kPFE)/wtmol(10)/convfac)*1.e-6
          else
            fe_mmr = ((f_FE_PRM*con(kFPRM)+
     &                 f_FE_CRS*con(kFCRS))/wtmol(10)/convfac)*1.e-6
          endif
        ...
        if (kPMN.lt.nspec+1 .or. (f_MN_PRM.ge.0. .and. f_MN_CRS.ge.0.)) then
          mn_ii  = 1.0    ! fraction partitioning of MN(II), GS 01July2011
          mn_sol = 0.5    ! solubility of Mn, GS 28July2011
          if (kPMN.lt.nspec+1) then
            mn_mmr = (con(kPMN)/wtmol(11)/convfac)*1.e-6
          else
            mn_mmr = ((f_MN_PRM*con(kFPRM)+
     &                 f_MN_CRS*con(kFCRS))/wtmol(11)/convfac)*1.e-6
          endif
        ...
        if (kPK.lt.nspec+1) then
          r_aer(9) = (con(kPK)/wtmol(6)/convfac)*1.e-6
        elseif (f_K_PRM.ge.0. .and. f_K_CRS.ge.0.) then
          r_aer(9) = ((f_K_PRM*con(kFPRM)+
     &                 f_K_CRS*con(kFCRS))/wtmol(6)/convfac)*1.e-6
        else
...
c
        if (lddm) then
          ...
          enddo
          call DDMRADINI (con(kso2),con(kna),con(kpcl),f_CA_CRS,convfac, !bk - broken
...
c
        if (lddm) then
          call DDMRADM (r_1,r_gas(9)/r_old(9),r_2,r_gas(10)/r_old(10), !bk - broken
     ...
     &                                       ,r_2,r_gas(10)/r_old(10)
            ...
            do ip = 1, nfam
              if (con(kna).gt.con(kpcl)) then
                r_1 = sddm(ip,jdpPCL )/wtmol(4)/convfac *1.e-6
              else
                r_1 = sddm(ip,jdpNA  )/wtmol(5)/convfac *1.e-6
              endif
              write(iout,*) ' SI(',ip,') = ',
     ...
     &            r_1,f_CA_CRS*sddm(ip,jdpFCRS)/wtmol(8)/convfac*1.e-6
            enddo
            call camxerr()
          endif
        endif
...
c
        ...
        if (kPK.lt.nspec+1) then
          xYPa(3) = DBLE(                     con(kpk )/wtmol(6) *1.e-6) ! K+    [mol/m3]
        elseif (f_K_PRM.ge.0. .and. f_K_CRS.ge.0.) then
     ...
     &                                                 /wtmol(6) *1.e-6)
        ...
        if (kPCA.lt.nspec+1) then
          xYPa(4) = DBLE(                     con(kpca)/wtmol(8) *1.e-6) ! Ca++  [mol/m3]
        elseif (f_CA_PRM.ge.0. .and. f_CA_CRS.ge.0.) then
     ...
     &                                                 /wtmol(8) *1.e-6)
        ...
        if (kPMG.lt.nspec+1) then
          xYPa(5) = DBLE(                     con(kpmg)/wtmol(9) *1.e-6) ! Mg++  [mol/m3]
        elseif (f_MG_PRM.ge.0. .and. f_MG_CRS.ge.0.) then
     ...
     &                  imask,1,1,jmeq,Dd)
        ...
        if (kPCA.lt.nspec+1) then
          dno3 = DBLE(con(kPCA)/wtmol(8)*1.e-6) ! half of CaCO3 is assumed to be replaced by Ca(NO3)2          
        elseif (f_CA_PRM.ge.0. .and. f_CA_CRS.ge.0.) then
...
c
        if (lddm) then
          call DDMISRINI (fca0,convfac,wtmol,sddm,nfam,nsen,NWTMOL,iout)
        endif
c
c========================== DDM End ====================================
c
...
c
        if (lddm) then
          call DDMISRPIA (fca0,convfac,wtmol,sddm,nfam,nsen,NWTMOL,ierr,
     &                                                             iout)
          if (ierr.lt.0) then
            write(iout,'(a,4i4)') ' igrd,i,j,k = ',
     &                                          igrdchm,ichm,jchm,kchm
            ...
            write(iout,*) ' fca0 ',fca0,(sddm(ip,jdpFCRS),ip=1,nfam)
            do ip = 1, nfam
              if (lno3_isr) then
                r_1 = (     sddm(ip,jdpHNO3)*convfac
     &                +     sddm(ip,jdpPNO3)/wtmol(3)
     &                -fca0*sddm(ip,jdpFCRS)/wtmol(8))*1.e-6
              else
                r_1 = 0.0
              endif
              write(iout,*) ' SI(',ip,') = ',
