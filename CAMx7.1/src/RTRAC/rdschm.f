...
c                               core model rates
c    11/13/20   --gyarwood--    Enable RTCMC to read deposition
c                               parameters
c
c-----------------------------------------------------------------------
c    Include files:
...
c
 ...
 100  read (iinp, '(A)', end=8001, err=8000) line
      ...
      if( letr .EQ. '#' ) then
         ...
         reacschm(isp) = 1.0
         rsclschm(isp) = 1.0
         read(line,*,ERR=8004) spnmschm(isp), letr, hlawschm(isp),
     &                         tfacschm(isp), molwschm(isp),
     &                         reacschm(isp), rsclschm(isp)
         if( letr .EQ. 'F' ) then
            itypschm(isp) = 1
