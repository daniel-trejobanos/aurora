...
c-----------------------------------------------------------------------
c
      integer   i, j, k, l, n, icount, itmp, itm2, itm3, mvecedge
      logical   lok
      character*10   sptmp
...
c
      do k = 1,ngasrtc
         ...
         elseif( spnmrt(k) .EQ. nam_CH4 ) then
           ...
           itmp = 0
           if( ngas .GT. 0 ) itmp = idxspc(spnmrt(k),spname,ngas)
           if( itmp .NE. 0 ) then
              idxfix(k) = 6 + itmp
              itypsp(k) = 4
           endif
...
c
      do n = 1,nrxnrtc
         do k = 1, nrct(n)
            idxrct(n,k) = idxspc(namrct(n,k),spnmrt,ngasrtc)
         enddo
         do k = 1,nprd(n)
            idxprd(n,k) = idxspc(namprd(n,k),spnmrt,ngasrtc)
         enddo
      enddo
c
...
c
               if( icount .EQ. 2 .AND. njactrm .GT. 1 ) then
                  if( idrxjac(njactrm) .EQ. idrxjac(njactrm-1) 
...
c
      if( MAX(ntotsp,ngasschm) .GT. MXTRSP ) then
        write(iout,'(//,A)') 'ERROR in GENCMC:'
...
c
      ...
      write(idiag,'(A,A)')'     Name      LowBnd (ppm)   H (M/atm)   ',
     &                    'Tfact (K)  MW (g/mol) Reactivity      Rscale'
ciiii aaaaaaaaaa  -e.eeeE-ee  -e.eeeE-ee-fffffff.fff-fffffff.fff-fffffff.fff-fffffff.fff
      do i = 1, ngasrtc
         ptname(i) = spnmrt(i)
         lsamap(i) = i
         lsagas(i) = .TRUE.
         ...
         do j = 1,ngasschm
           if (spnmrt(i).eq.spnmschm(j)) then
             rthlaw(i) = hlawschm(j)
             rttfact(i) = tfacschm(j)
             rtmolwt(i) = molwschm(j)
             rtdrate(i) = sqrt(molwschm(j)/18.)
             rtreact(i) = reacschm(j)
             rtscale(i) = rsclschm(j)
           endif
         enddo
c
         write(idiag,'(i4,1x,a10,2e12.3,4f12.3)') 
     &         i,spnmrt(i),rtlbnd(i),rthlaw(i),rttfact(i),rtmolwt(i),
     &         rtreact(i),rtscale(i)
      enddo
      write(idiag,*)
c
