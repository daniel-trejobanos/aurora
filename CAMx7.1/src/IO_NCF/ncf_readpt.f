...
c
      do ipt=1,nptsrc_files(ifile)
          if( plume_bot(ipt) .GT. 0. .AND. plume_top(ipt) .LE. 0. ) goto 7002
          if( plume_bot(ipt) .LE. 0. .AND. plume_top(ipt) .GT. 0. ) goto 7002
          if( plume_bot(ipt) .GT. 0. .AND. plume_bot(ipt) .GE. plume_top(ipt) ) goto 7002
          if( plume_bot(ipt) .GT. 0. ) effph(ipt+idx_start) = -plume_bot(ipt)
          if( plume_top(ipt) .GT. 0. ) flowrat(ipt+idx_start) = -plume_top(ipt)
      enddo
c
