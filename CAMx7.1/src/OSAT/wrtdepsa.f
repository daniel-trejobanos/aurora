      subroutine wrtdepsa(tim2,idat2,igrd,iunit,nox,noy,nspdep,
     &                                               dryfld,wetfld)
      use grid
      use chmstry
      use camxcom
...
c
     ...
     &               ((dryfld(i,j,l),i=ibegcel,iendcel),j=jbegcel,jendcel)
        endif
      enddo
c
...
c
      ...
      do l = 1,notimespc
        if( loutsa(l) ) then
           write(iunit) nseg,(ispec(n,l),n=1,10),
     &               ((wetfld(i,j,l),i=ibegcel,iendcel),j=jbegcel,jendcel)
        endif
      enddo
c
      return
