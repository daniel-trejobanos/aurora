      ...
      subroutine addrcp(igrd,nox,noy,noz,nspsa,saconc)
      use grid
      use tracer
c
c----CAMx v7.10 210105
...
c
          if( idrcp(ircp) .EQ. IDCEL ) then
              ...
              jcl = jrecep(ircp,1)
              do i=1,ntotsp
                 conrcp(i,ircp) = conrcp(i,ircp) + saconc(icl,jcl,1,i)
              enddo
c
...
c
     ...
     &                               saconc(icl,jcl,1,i) / nclrcp(ircp)
                 enddo
              enddo
c
...
c
     ...
     &                                           saconc(ixhigh,iylow,1,i)
                  xylow = xylow + (xhigh - recepx(ircp)) / xcelwd * 
     &                                          saconc(ixlow,iylow,1,i)
                  xyhigh = (recepx(ircp) - xlow) / dxcell * 
     &                                          saconc(ixhigh,iyhigh,1,i)
                  xyhigh = xyhigh + (xhigh - recepx(ircp)) / 
     &                                dxcell * saconc(ixlow,iyhigh,1,i)
c
