...
c-----------------------------------------------------------------------
c
c  --- if the is no receptor file then just return ---
c
...
c
      ...
      if( rcptmp .EQ. CDPNT ) then
          ...
          write(iout,'(//,a)') 'ERROR in RERCP:'
          write(iout,'(/,1X,4A)') 'Invalid receptor ',
     &        'code found in receptor definition file: ',
     &                               rcptmp(:istrln(rcptmp))
