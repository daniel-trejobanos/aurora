C**** WALK1PUF
c
      subroutine walk1puf(dt,igrd,nx,ny,nz,iipig,jjpig,ingrd,lkick,
...
c       6/14/10              Fix to alleviate occasional trapping of puffs
c                            at nested grid boundaries with converging winds.
c      10/29/20              Adjustments for single-precision error in
c                            puff position outside of (iipig,jjpig) cell
c
c     Input arguments:
...
c        lkick               kick flag for too many iterations
...
c
      include "camx.prm"
c
      integer igrd,nx,ny,nz,iipig,jjpig,ingrd
...
c
      ...
      xcell0 = min( max(xcell0,deltax(j,igrd)*0.001),deltax(j,igrd)*0.999 )
      ycell0 = min( max(ycell0,deltay(igrd)*0.001),deltay(igrd)*0.999 )
c
c-----Compute the new position after the whole time step
c
      ax = uum
      bx = (uup - uum)/deltax(j,igrd)
      if (abs(bx).lt.1.e-4) then
        ax = ax + bx*xcell0
        xcell = xcell0 + ax*dt
      else
...
c     the time needed to reach the boundary (+ some fuzz)
c
      dtx = dt
      dty = dt
