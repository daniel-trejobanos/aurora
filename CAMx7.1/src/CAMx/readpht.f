...
c        11/17/20      Moved definition of TUV photolysis file labels to
c                      READCHM.F
c
c     Input arguments:
c        none
...
c
      ...
      if (record(1:20).ne.tuvlabl(idmech)) goto 7004
...
c
      ...
      write(idiag,'(a15,99f7.0)') 'Ozone classes :',(ozcl(i),i=1,NOZN)
...
c
 7000 continue
     ...
     &                 ' number of reactions?'
      write(iout,'(/,2A)') ' Make sure the photolysis rate file ',
     &                 'matches the mechanism you are running.'
      call camxerr()
c
 7001 continue
      write(iout,'(//,A)') 'ERROR in READPHT:'
...
c
 7004 continue
     ...
     &                  ' Reading header record at line: ', line
      ...
      write(iout,'(a,a)') '   Expecting: ',tuvlabl(idmech)
      write(iout,'(a,a)') '       Found: ',record(1:20)
      write(iout,'(2a,/,a)') ' Make sure you are using the correct ',
     &                       'version of the TUV processor for this',
     &                    ' version of CAMx and chemistry mechanism.'
      call camxerr()
c
c----------------------------------------------------------------------
c    Return point:
