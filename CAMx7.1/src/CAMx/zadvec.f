     ...
     &                  losat,ltopcon,igrd,ncol,nrow,nlay,nrads,nspc,
     &                  nsen,delt,dx,
...
c$omp&          convfac,ls,isen,ioff,idx,sen1d,fluxtop,
...
c
     ...
     &                                     .AND. j .LE. jz ) then
                   if (fluxtop.lt.0) then
                     fluxtmp(i,j,1,ispc) = -fluxtop*dx(j+j0)*dy*delt
                   else
                     fluxtmp(i,j,2,ispc) = -fluxtop*dx(j+j0)*dy*delt
                   endif
               endif
            endif
c
