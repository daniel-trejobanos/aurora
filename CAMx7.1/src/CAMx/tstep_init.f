      ...
      if (iproc_id .EQ. 0) then 
         ...
         do igrd = 1,ngrid
            if( is_netcdf_i3dmet(igrd) ) then
                call ncf_readinp_2d(igrd,ncol(igrd),nrow(igrd),
     &                   tsurf(iptr2d(igrd)),pspt(iptr2d(igrd)),
     &                   tsnxt(iptr2d(igrd)),snow(iptr2d(igrd)),
     &                   snowage(iptr2d(igrd)),snowrat(iptr2d(igrd)))
                call ncf_readinp_3d(igrd,ncol(igrd),
     &                        nrow(igrd),nlay(igrd),
     &                        height(iptr3d(igrd)),phpt(iptr3d(igrd)),
     &                        hnxt(iptr3d(igrd)),press(iptr3d(igrd)),
     &                        pppt(iptr3d(igrd)),pnxt(iptr3d(igrd)),
     &                        windu(iptr3d(igrd)),pupt(iptr3d(igrd)),
     &                        unxt(iptr3d(igrd)),windv(iptr3d(igrd)),
     &                        pvpt(iptr3d(igrd)),vnxt(iptr3d(igrd)),
     &                        tempk(iptr3d(igrd)),ptpt(iptr3d(igrd)),
     &                        tnxt(iptr3d(igrd)),water(iptr3d(igrd)),
     &                        pwpt(iptr3d(igrd)),wnxt(iptr3d(igrd)))
                call ncf_readinp_cld(igrd,ncol(igrd),
     &                        nrow(igrd),nlay(igrd),
     &                        cwc(iptr3d(igrd)),pwr(iptr3d(igrd)),
     &                        pws(iptr3d(igrd)),pwg(iptr3d(igrd)),
     &                        cod(iptr3d(igrd)),cph(iptr3d(igrd)),
     &                        cigfrc(iptr2d(igrd)),cigtim(iptr2d(igrd)),
     &                        cigwtr(iptr3d(igrd)),cigpcp(iptr3d(igrd)),
     &                        cigent(iptr3d(igrd)),cigdet(iptr3d(igrd)))
                call ncf_readinp_kv(igrd,ncol(igrd),
     &                        nrow(igrd),nlay(igrd),
...
c
      if (nsteps .EQ. 1) then
         if (iproc_id .EQ. 0) then
            if( .NOT. lrstrt .AND. is_netcdf_iic ) then
               call ncf_readcnc()
            else
               call readcnc()
            endif
...
c
            if (ltrace .AND. .NOT. lrstrt) then
               if (tectyp .EQ. RTRAC .OR. tectyp .EQ. RTCMC) then
                  ...
                  call ncf_rdicrt(ncol(1),nrow(1),nlay(1),ntotsp,ptconc(1))
c
...
c
            if((lddm.OR.lhddm) .AND. .NOT.lrstrt .AND. nicddm.GT.0) then
               ...
               call ncf_rdicddm(ncol(1),nrow(1),nlay(1),ntotsp,ptconc(1))
c
c  --- interpolate initial conditions to nests ---
c 
               if (ngrid.gt.1) then
