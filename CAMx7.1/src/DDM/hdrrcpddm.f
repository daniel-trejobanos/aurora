...
c
     ...
     &                                        nspec*ngroup*nregin*nemddm
      write(iowrcp,9000,ERR=7000)
c
      write(iowrcp,9002,ERR=7000) 'Number of rate constant groups     ',
     &                                                          nrateddm
      write(iowrcp,9003,ERR=7000) 'Influencing rate constant groups   '
      if (nrateddm.gt.0)
     &        write(iowrcp,9003,ERR=7000)  (rateddm(i),i=1,nrateddm)
      write(iowrcp,9000,ERR=7000)
      write(iowrcp,9002,ERR=7000) 'Number of rate const sensitivities ',
     &                                                    nspec*nrateddm
      write(iowrcp,9000,ERR=7000)
c
      write(iowrcp,9002,ERR=7000) 'Number of rate term groups     ',
     &                                                          ntermddm
      write(iowrcp,9003,ERR=7000) 'Influencing rate term groups   '
      if (ntermddm.gt.0)
     &        write(iowrcp,9003,ERR=7000)  (termddm(i),i=1,ntermddm)
      write(iowrcp,9000,ERR=7000)
      write(iowrcp,9002,ERR=7000) 'Number of rate term sensitivities ',
     &                                                    nspec*ntermddm
      write(iowrcp,9000,ERR=7000)
c
      write(iowrcp,9002,ERR=7000) 'Number of HDDM sensitivity groups  ',
     &                                                             nhddm
      write(iowrcp,9003,ERR=7000) 'Influencing HDDM sens parameters   '
      if (nhddm.gt.0) then
        do i=1,nhddm
          if (i.gt.1) write(iowrcp,9003,ERR=7000)
     &                            '                                   '
          write(iowrcp,9003,ERR=7000) hddmsp(1,i),hddmsp(2,i)
          if (i.lt.nhddm) write(iowrcp,9000,ERR=7000)
        enddo
      endif
      write(iowrcp,9000,ERR=7000)
      write(iowrcp,9002,ERR=7000) 'Number of HDDM sensitivities       ',
     &                                                       nspec*nhddm
      write(iowrcp,9000,ERR=7000)
c
