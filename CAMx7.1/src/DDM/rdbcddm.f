...
c     11/28/16   --bkoo--       Added code to reset BC sens
c                               Fixed a time check bug
c
c-----------------------------------------------------------------------
c    Include files:
c-----------------------------------------------------------------------
...
c
      ...
      ierr = ncf_chkfile(iorbc,bcfil,action,bnfil)
...
c
     ...
     &                                                 nicddm + ioff - 1
                                  if( conwst(k,j) .GT. bdnl(imod) ) then
                                    saconc(1,j,k,iptr) = conwst(k,j) * convfac
                                  else
                                    saconc(1,j,k,iptr) = 0.0
                                  endif
c
...
c
     ...
     &                               tpgrd(nx,j,k)*prgrd(nx,j,k)/1013.
                                  ...
                                  iptr = iptddm(imod) + (iddm-1)*nedge +
     &                                                 nicddm + ioff - 1
                                  if( conest(k,j) .GT. bdnl(imod) ) then
                                    saconc(nx,j,k,iptr) = conest(k,j) * convfac
                                  else
                                    saconc(nx,j,k,iptr) = 0.0
                                  endif
                            enddo
c
...
c
     ...
     &                               tpgrd(i,1,k)*prgrd(i,1,k)/1013.
                                  ...
                                  iptr = iptddm(imod) + (iddm-1)*nedge +
     &                                                 nicddm + ioff - 1
                                  if( consth(k,i) .GT. bdnl(imod) ) then
                                    saconc(i,1,k,iptr) = consth(k,i) * convfac
                                  else
                                    saconc(i,1,k,iptr) = 0.0
                                  endif
c
c  --- load the east boundary ---
c
...
c
     ...
     &                               tpgrd(i,ny,k)*prgrd(i,ny,k)/1013.
                                  ...
                                  iptr = iptddm(imod) + (iddm-1)*nedge +
     &                                                 nicddm + ioff - 1
                                  if( connth(k,i) .GT. bdnl(imod) ) then
                                    saconc(i,ny,k,iptr) = connth(k,i) * convfac
                                  else
                                    saconc(i,ny,k,iptr) = 0.0
                                  endif
                            enddo
                         enddo
                      endif
